...
| 2026-08-22 04:0x | `0e4da44` | Created the `submission-final` tag, this changelog, and the README pointer to both. No content change to the project itself. |
| 2026-08-22 04:1x | (this commit) | Published the S3.2 paired-pool result onto the user-value page and the bounded-fee-hook capability, which the experiment's own pre-registration required whichever way the result landed. The site gives the 100.195% ratio its own tile beside the 195.38 bps figure, because the ratio is what makes it a conformance test rather than a discovery. Closed the score-improvement tracker and opened its successor, `docs/.../04-planning/tinjau-post-submission-score-tracker.md`. |
| 2026-08-22 | (this commit) | Added `docs/.../07-submission/EVALUATION-DOSSIER-2026-08-22.md`, a neutral evaluation dossier written for an evaluator with no prior knowledge. It supersedes the now-stale `EVALUATE-TINJAU.txt` as a current description, gives no scores and no verdict, reproduces the project's own claim boundary and its unfavourable findings at full weight, and includes a section of questions designed to go against the project. `EVALUATE-TINJAU.txt` is left unedited as historical evidence of what an earlier evaluator was given. |
| 2026-08-22 | (this commit) | Added the seven published judging criteria, verbatim with their source, to the evaluation dossier: stripping conclusions had also stripped the criteria an evaluator needs to score against. Added `EVALUATE-PROMPT-2026-08-22.txt`, the companion instruction for an evaluating agent, which tells it to treat the dossier as an interested party's submission, to rank its own observations above it, and to report any discrepancy it finds between the two as a finding. |
