    ...
    <div className="min-h-screen bg-paper text-coal">
      ...
      <section className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 lg:px-8 lg:py-28" aria-labelledby="deployment-ledger-title">
        <div className="grid gap-8 lg:grid-cols-[0.45fr_1fr]">
          <div>
            <h2 id="deployment-ledger-title" className="font-display text-section-sm text-coal">
              Deployment ledger
            </h2>
            <p className="mt-4 max-w-md text-body-sm text-coal-muted">
              Read straight from <span className="font-data">deployed-addresses.json</span>, not
              retyped. Every contract below has bytecode on chain, checked with{" "}
            ...
            <p className="mt-5 rounded-lg border-2 border-watch bg-watch/[0.07] p-4 text-body-sm text-coal">
              {DEPLOYED_STATUS_TEXT}
            </p>
            <p className="mt-4 rounded-lg border border-edge-light bg-paper-soft p-4 text-body-sm text-coal-muted">
          ...
          <div>
            {DEPLOYED_STACKS.map((stack) => (
              <div key={stack.stackId} className="mb-10 border-t border-coal">
                <div className="flex flex-wrap items-baseline justify-between gap-2 py-4">
                  <h3 className="font-display text-heading-sm text-coal">{stack.label}</h3>
                  <p className="data-label text-coal-faint">
                ...
                {stack.contracts.map((contract) => (
                  <article
                    ...
                    className="grid gap-4 border-t border-edge-light py-5 md:grid-cols-[0.7fr_1fr_auto]"
                  >
                    <div>
                      <h4 className="font-body text-body-sm font-medium text-coal">
                        {contract.role}
                      </h4>
                      <p className="data-label mt-1 text-coal-faint">
                        {contract.isBuilderControlled ? "Builder-controlled" : "Third-party"}
                      </p>
                    </div>
                    <div>
                      <p className="break-all font-data text-[11px] text-coal">
                        {contract.address}
                      </p>
                      {contract.note ? (
                        <p className="mt-2 text-body-xs text-coal-muted">{contract.note}</p>
                      ) : null}
                    </div>
                    <div className="flex items-start gap-3 md:justify-end">
                      <span className="data-label whitespace-nowrap text-normal-deep">
                      ...
                      <a
                        ...
                        className="inline-flex min-h-10 items-center border-b border-coal font-data text-[10px] font-semibold uppercase tracking-[0.06em]"
                      >
                        Explorer
                      </a>
                    </div>
