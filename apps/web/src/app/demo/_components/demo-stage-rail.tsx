        ...
        {mission.stages.map((stage) => {
          ...
          return (
            <li key={stage.id} className="border-r border-edge last:border-r-0">
              <button type="button" disabled={locked} onClick={() => dispatch({ type: "STAGE_REVISITED", stageId: stage.id })} aria-current={current ? "step" : undefined} title={locked ? "Complete the current objective to unlock this stage" : undefined} className={`flex min-h-16 min-w-36 items-center gap-3 px-4 text-left transition-colors duration-150 ease-tinjau ${current ? "bg-signal text-black" : complete ? "bg-surface text-white hover:bg-surface-raised" : "cursor-not-allowed bg-canvas text-ink-muted opacity-55"}`}>
