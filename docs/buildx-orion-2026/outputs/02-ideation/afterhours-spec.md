...
- **Two different v4 environments, because canonical v4 does not exist on testnet 1952 (§7).**
  - **Mainnet 196:** canonical PoolManager `0x360e68…fb32`, hook + seeded pool deployed against it. This is the launch that satisfies the Event A mainnet requirement.
...
- **(c) Markout study:** realised LP losses on the **existing v3 wNVDAx pool** on the first trades after each event in (b), computed from v3 Swap events pulled by RPC in the same targeted windows. This is where the dollars are, and after the rename it is the headline, not (b).
...
**Execution order — day 1 is eligibility, before any feature code:** create the X account; fund the wallet with OKB (mainnet) and claim testnet OKB from the faucet; execute a verified mainnet purchase of wNVDAx and a seed transaction; deploy your own PoolManager on testnet 1952 (§4.3); create Event B assets (website shell, GitHub, Discord/Telegram). These are the only items that produce *ineligibility* rather than a low score. (The facts that previously gated day 1 — wMSTRx mechanics, price-history depth, v4-on-testnet — were all resolved on 2026-08-17 and are recorded in §7.)
...
- **Mandatory (any failure = ineligible):** AI in product design, deployed on X Layer; testnet + mainnet launch during the hackathon; dedicated active X account; submission post from that account mentioning @XLayerOfficial; form by deadline.
...
- Prizes 30,000 / 15,000 / 5,000 USDT, plus a separately judged **AI-RWA Liquidity Grant of 50,000 USDT** (criteria: product quality, innovation, user value, contribution to ecosystem; grant must fund the project's growth on X Layer). No track selector on the form → the submission text **states AI-RWA membership explicitly** and includes verifiable mainnet tx links.
...
1. **Reaction latency is now measured on the full pre-registered sample (median 4.6 min, n=46, §7), but the dollar consequence is not.** §4.8c has not been run. If LP losses on the first post-filing trade turn out to be trivial (say $20 on a $221k pool), the whole chain is correct and unimportant, and that must be said plainly. The reframe in §4.8b fixed the *validity* of the claim, not its *magnitude*.
...
4. Markout study result may be small — and it is published regardless.
...
11. Eligibility items (X account, funded wallet, mainnet launch) are scheduled but not done; they are the only fatal-risk items.
