...
- Required technology: `[confirmed]` Event A — the project **must** include AI and **must** deploy on X Layer Testnet **during** the hackathon (i.e. by the 2026-08-21 23:59 UTC submission deadline). Mainnet deployment is **not** a submission-deadline requirement — the official FAQ explicitly answers "Does the project need to be deployed on Mainnet during the Hackathon?" with "The project must be deployed on the X Layer Testnet during the Hackathon and **subsequently** launched on the X Layer Mainnet" (REF-027, verified 2026-08-17). **Read carefully, and note this line was overconfident until 2026-08-21:** it previously concluded that "subsequently" places mainnet after the hackathon window, "most likely tied to grant/prize disbursement rather than judging eligibility", and that "testnet-only is sufficient to submit and be judged". That is an `[inferred]` reading presented as `[confirmed]`, and the source does not carry it. What the page actually says, verbatim and re-verified against the raw HTML on 2026-08-21: *"During the Hackathon, the project must be deployed on the X Layer Testnet and subsequently launched on the X Layer Mainnet."* Two facts constrain the reading in opposite directions: the clause carries **no date** for the mainnet leg, and it sits inside the eligibility block whose preamble reads *"meet any requirement will be ineligible to participate in the Hackathon or receive prizes."* So mainnet is a **stated obligation with an unpublished deadline**. Whether it gates judging eligibility is **not determinable from published material**. Testnet-only being sufficient to be judged is a hope, not a finding. Event B — must be a working AI agent; registration/submission wallet must be on Base (REF-001, REF-002)
- Allowed technology: `[confirmed]` X Layer is full-EVM-equivalent OP Stack, so any standard Solidity/EVM tooling works. Testnet chain ID **1952**, RPC `https://testrpc.xlayer.tech/terigon`, gas token OKB, explorer `https://web3.okx.com/explorer/x-layer-testnet` (verified 2026-08-17 — the older `www.okx.com/web3/explorer/xlayer-test` path still works but 301-redirects here; use the canonical form directly). Mainnet chain ID **196**, RPC `https://rpc.xlayer.tech` (REF-005)
...
- Judging criteria and weights: `[confirmed]` Event A — **seven criteria**, verbatim from Terms & Conditions clause 4: *"application of AI, innovation, product completeness, user value, integration with X Layer, growth potential, and contribution to the X Layer ecosystem"*. No weights published; Organizer's decisions are final. **Correction, itself corrected 2026-08-21:** an earlier note here said "code quality", "onchain data" and "market potential" appear only in the spreadsheet extraction and "not in the official terms", and instructed that they not be treated as criteria. **That was wrong**, and it was wrong because the page carries *two* separately numbered clause-4s and only the first was read. Terms clause 4 ("Judging") gives the seven criteria above. A second block, the Disclaimer, has its own clause 4 ("Final Determination"), verbatim from the official page on 2026-08-21: *"The Organizer will consider onchain data, code quality, innovation, and market potential. Final rankings will be determined at the Organizer's sole discretion."* So all three phrases are official and **are** decision-relevant; the seven-criteria list is simply not the only published statement about how projects are ranked. Liquidity Grant is judged separately on *"overall performance during the Hackathon, including product quality, innovation, user value, and contribution to the ecosystem"*, awarded to one best-performing project in the AI-RWA track. Event B — partner judges score 0–10 on **usefulness, execution, originality**, informed by Orion's AI vetting score and community upvotes from registered builders; judges decide (REF-001, REF-002)
