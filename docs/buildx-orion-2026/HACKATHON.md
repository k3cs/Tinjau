...
- Required technology: `[confirmed]` Event A — the project **must** include AI and **must** deploy on X Layer Testnet **during** the hackathon (i.e. by the 2026-08-21 23:59 UTC submission deadline). Mainnet deployment is **not** a submission-deadline requirement — the official FAQ explicitly answers "Does the project need to be deployed on Mainnet during the Hackathon?" with "The project must be deployed on the X Layer Testnet during the Hackathon and **subsequently** launched on the X Layer Mainnet" (REF-027, verified 2026-08-17). "Subsequently" places mainnet launch after the hackathon window, most likely tied to grant/prize disbursement rather than judging eligibility — testnet-only is sufficient to submit and be judged. Event B — must be a working AI agent; registration/submission wallet must be on Base (REF-001, REF-002)
